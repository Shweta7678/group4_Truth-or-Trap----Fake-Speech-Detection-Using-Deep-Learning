    ...
    with open(save_path, 'w') as fh:
        for batch_x, batch_y in data_loader:
            ...
            with torch.no_grad():
                ...
                _, predicted = torch.max(softmax_probs, 1)
                predicted_class_probs = softmax_probs.gather(1, predicted.unsqueeze(1)).squeeze(1)

            for i in range(batch_x.size(0)):
                pred_label = predicted[i].item()
                true_label = batch_y[i].item()
                conf_score = predicted_class_probs[i].item()
                ...
                if pred_label == 0:
                    count_spoof += 1
                    fh.write(f"\nPrediction: spoofed | Confidence: {conf_score:.4f} | True: {'spoofed' if true_label==0 else 'bonafide'}")
                else:
                    count_bonafide += 1
                    fh.write(f"\nPrediction: bonafide | Confidence: {conf_score:.4f} | True: {'bonafide' if true_label==1 else 'spoofed'}")

    # Compute metrics
    accuracy = accuracy_score(all_labels, all_preds)
    f1 = f1_score(all_labels, all_preds, average='binary')  # use average='macro' if multi-class
    report = classification_report(all_labels, all_preds, target_names=["spoofed", "bonafide"])
    cm = confusion_matrix(all_labels, all_preds)

    print(f"\nEvaluation Completed")
    print(f"Total spoofed predicted: {count_spoof}")
    print(f"Total bonafide predicted: {count_bonafide}")
...
if __name__ == '__main__':
    ...
    # Dataset
    parser.add_argument('--database_path', type=str, default='/home/pm_students/SSL_Anti-spoofing/archive/for-norm/for-norm/ASVspoof2021_LA_eval', help='Change this to user\'s full directory address of LA database (ASVspoof2019- for training & development (used as validation), ASVspoof2021 DF for evaluation scores). We assume that all three ASVspoof 2019 LA train, LA dev and ASVspoof2021 DF eval data folders are in the same database_path directory.') 
    '''
    % database_path/
    %   |- DF
